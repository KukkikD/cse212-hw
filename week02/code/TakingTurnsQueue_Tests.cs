...
public class TakingTurnsQueueTests
{
    ...
    // Defect(s) Found: Failed. The queue does not follow FIFO order (Sue is returned before Bob).
    ...
    // Defect(s) Found: Failed. The queue order is incorrect after adding a player (not FIFO).
    ...
    // Run 10 times.
    // Expected Result: Bob, Tim, Sue, Bob, Tim, Sue, Tim, Sue, Tim, Tim
    // Defect(s) Found: Failed. Infinite-turn players (0) are not re-enqueued correctly.
    ...
    // Defect(s) Found: Infinite-turn players (negative) are not re-enqueued correctly.    
    ...
    // Defect(s) Found: Passed. An InvalidOperationException is thrown with the correct message.
